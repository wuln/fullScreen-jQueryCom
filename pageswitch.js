(function($){
	...
	//点击分页的事件代理
	$('body').delegate('#pages li', 'click', function(e) {
		var _id=$(this).attr('node-id');
		if (iIndex != _id) {
			iIndex=_id;
			scrollPage(arrElement[iIndex]);
		} 
	...
	function initPagination(){
		var length = sections.length;
		if(length){
			var pageHtml = '<ul id="pages"><li class="active" node-id="0"></li>';
			for(var i=1;i<length;i++){
				pageHtml += '<li node-id="' + i + '"></li>';
			}
			pageHtml += '</ul>';
	...
	function paginationHandler(){
		var pages = $("#pages li");
		pages.eq(iIndex).addClass("active").siblings().removeClass("active");//链式调用
	}
